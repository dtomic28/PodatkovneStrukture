...
TEST_CASE("poisciZBisekcijo functionality")
{
    ...
    int largeArraySize = 10000000;
    int* largeArray = new int[largeArraySize];
    std::fill_n(largeArray, largeArraySize, 2);
    ...
    {
        std::sort(testArray, testArray + n);              // Ensure sorted for binary search
        REQUIRE(poisciZBisekcijo(3, testArray, n) >= 0);  // Specific index might vary based on implementation
    }

    SECTION("does not return error on empty array")
    {
        REQUIRE(poisciZBisekcijo(3, nullptr, 0) == -1);
    }

    SECTION("works for 10 million elements")
    {
        std::sort(largeArray, largeArray + largeArraySize);  // Ensure sorted for binary search
        std::reverse(largeArray, largeArray + largeArraySize);
        REQUIRE(poisciZBisekcijo(1, largeArray, largeArraySize) == largeArraySize - 1);
    }
